...
def build_cotrend(datas, df):
    ...
    return ('<div class="card"><h2>검색량 · 수출 · 매출 동행</h2>'
...
def build_amazon(datas):
    ...
    df = amazon.load()
    if df.empty:
        return ('<div class="card"><h2>아마존 Best Sellers 순위</h2>'
...
<header>
  ...
  <div class="sub">수출 기준월 {asof} · 생성 {generated}
    · <a href="../" style="color:var(--accent)">S-Oil 트래커</a></div>
</header>

{banner}

<div class="card">{pxrow}</div>
